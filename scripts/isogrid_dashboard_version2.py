...
let GEOM = {S:Math.sqrt(3), dz_fac:Math.sqrt(3)/2, theta:Math.PI/3, type:'tri_0', orient:0, fams:[0,Math.PI/3,-Math.PI/3], axial_EA:1.0, axial_EI:1.0}; // updated from state
...
function drawPreviewIfNeeded(state){
  ...
  if(type==='honeycomb'){
    ...
    const s_px = a_mm*s;
    // base families 0° and 90°, then add orient (0° or 45°)
    const angles = [0+orient, Math.PI/2+orient];
    ...
    // Tri-hexagonal (Kagome-like): continuous ±60° lines, thicker horizontal bars
    const s_px = a_mm*s; const centerX = c.width/2, centerY = c.height/2; const L = Math.hypot(c.width, c.height);
    const angsDiag = [GEOM.theta, -GEOM.theta].map(a=>a+orient);
    angsDiag.forEach(phi=>{
      const ux=Math.cos(phi), uy=Math.sin(phi);
      for(let k=-12;k<13;k++){
        const px = centerX + k*s_px*(-uy); const py = centerY + k*s_px*(ux);
        ctx.beginPath(); ctx.moveTo(px - ux*L, py - uy*L); ctx.lineTo(px + ux*L, py + uy*L); ctx.stroke();
      }
    });
    // horizontal bars thicker
    const oldLW = ctx.lineWidth; ctx.lineWidth = oldLW*1.8;
    const ux= Math.cos(0+orient), uy=Math.sin(0+orient);
    for(let k=-10;k<11;k++){
      const px = centerX + k*s_px*(-uy); const py = centerY + k*s_px*(ux);
      ctx.beginPath(); ctx.moveTo(px - ux*L, py - uy*L); ctx.lineTo(px + ux*L, py + uy*L); ctx.stroke();
    }
...
function mount(){
  ...
  function clearMarker(){ state.bestMarker = null; document.getElementById('optOut').textContent = ''; }
  ...
  // Default objective: Ncr/m
  objSel.value = 'max_ncr_over_m';
